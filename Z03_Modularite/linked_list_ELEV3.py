...
def taille(linked:LinkedList) -> int:
    """ Renvoie la taille de la liste chaînée (nombre de maillons):
            Ex:      a   <=>   1 -> 22 -> 34 -> None
                     taille(a) == 3
    """
    # cas de base : si la liste chaînée est vide, on renvoie le cas de base
    longueur = 0
    if est_vide(linked):
        return longueur

    # cas récursif : si la liste chaînée n'est pas vide -> cas récursif
    else:
    #   couper : fonction queue(linked)
        couper = queue(linked)
    #   régner : appel récursif
...
def somme(linked:LinkedList) -> int:
    """ Renvoie la somme des valeurs de la liste chaînée:
            Ex:      a   <=>   1 -> 22 -> 34 -> None
                     somme(a) == 57
    """
    # cas de base : si la liste chaînée est vide :
    couper = 0
    if est_vide(linked):
        return couper

    # cas récursif : si la liste n'est pas vide
    else:
