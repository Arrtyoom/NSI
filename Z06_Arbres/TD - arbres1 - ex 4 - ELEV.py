...
def exist(tree, path):
    ...
    #         renvoie Vrai (on a tt validé avant !)
    #     Fin Si
    #     Pour chaque sous-arbre de la liste d'enfants:
    #         Si le nom du sous-arbre correspond au segment recherché:
    #             renvoie valid(enfant du sous arbre)
    #         Fin Si
    #     Fin Pour
    #     renvoie Faux
    # Fin Fonction

    def valid(children:list, i:int) -> bool:
        """ (question 8)
            La fonction valid vérifie qu'un enfant de la liste d'enfants
            contient le segment en cours

            @children:  liste de noeuds (sous-arbres)
            @i:         indice du segment en cours dans lst_noms
            @return:    booléen indiquant si le chemin est valide
        """
        if i >= len(lst_noms):
            return True
        for child in children:
            if child[0] == lst_noms[i]:
                return valid(child[1], i + 1)
        return False

    return valid(children, 0)
